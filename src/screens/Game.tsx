...
export function Game({ mode, onExit, myTheme, opponentTheme, room, isHost = true }: GameProps) {
  ...
  const measureLayout = useCallback(() => {
    ...
    if (pilesRef.current) {
      ...
      pileEls.forEach((el) => {
        ...
        newLayout.pilePositions.push({ x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 });
      });
    }

    if (p1HandRef.current) {
      const rect = p1HandRef.current.getBoundingClientRect();
      newLayout.p1HandCenter = { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
    }

    if (p1FaceDownRef.current) {
      const rect = p1FaceDownRef.current.getBoundingClientRect();
      newLayout.p1FaceDownCenter = { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
    }

    if (p2HandRef.current) {
      const rect = p2HandRef.current.getBoundingClientRect();
      newLayout.p2HandCenter = { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
    }

    if (p2FaceDownRef.current) {
      const rect = p2FaceDownRef.current.getBoundingClientRect();
      newLayout.p2FaceDownCenter = { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
    }
  ...
  const triggerFaceDownPlayAnimation = useCallback((slotIndex: number, pileIndex: number, player: 'P1' | 'P2') => {
    ...
    const faceDownCenter = player === 'P1' ? layout.p1FaceDownCenter : layout.p2FaceDownCenter;
    ...
    const replacementCard = !isSuccess && state.deck.length > 0 
          ...
          <div className="relative flex items-center justify-center">
            ...
            <motion.div 
              ...
              style={{
                ...
                boxShadow: state.deck.length > 0 ? `0 4px 12px ${myTheme.neutral.glow}` : 'none',
              }}
            >
              {state.deck.length > 0 && (
                <>
              ...
              {state.centerPiles.map((_, index) => {
                ...
                return (
                  <div
                    ...
                    onClick={hasSelection && isPlayerTurn && isLegal ? () => handleSelectPile(index) : undefined}
                  />
                );
              })}
            </div>
      ...
      {layout && (
        <CardLayer
          ...
          hiddenCardIds={[
            ...
            faceDownPlay?.phase === 'replacing' ? faceDownPlay.replacementCard?.id : undefined,
        ...
        {faceDownPlay && (
          <>
            ...
            <motion.div
              ...
              animate={{
                x: faceDownPlay.phase === 'retreating' || faceDownPlay.phase === 'replacing' || faceDownPlay.phase === 'done'
                  ? faceDownPlay.handPos.x - 32
                  : faceDownPlay.pilePos.x - 32,
                y: faceDownPlay.phase === 'retreating' || faceDownPlay.phase === 'replacing' || faceDownPlay.phase === 'done'
                  ? faceDownPlay.handPos.y - 48
                  : faceDownPlay.pilePos.y - 48 + (faceDownPlay.phase === 'revealed' && !faceDownPlay.isSuccess ? -10 : 0),
                zIndex: 200,
              }}
              exit={{ opacity: 0 }}
              transition={{
              ...
              >
                ...
                <div className="absolute inset-0" style={{ backfaceVisibility: 'hidden' }}>
                  <Card 
                    card={null} 
                    faceUp={false}
                    backColor={faceDownPlay.player === 'P1' ? myTheme.primary : opponentTheme.secondary}
                  />
                </div>
                {/* Front face */}
                <div 
            ...
            {faceDownPlay.phase === 'replacing' && faceDownPlay.replacementCard && layout && (
              <motion.div
